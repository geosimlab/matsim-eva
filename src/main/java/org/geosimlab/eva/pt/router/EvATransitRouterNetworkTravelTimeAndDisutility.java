...
public class EvATransitRouterNetworkTravelTimeAndDisutility implements TravelTime, TransitTravelDisutility {
	...
	public EvATransitRouterNetworkTravelTimeAndDisutility(final TransitRouterConfig config, PreparedTransitSchedule preparedTransitSchedule) {
		this.config = config;
	...
	public double getLinkTravelTime(final Link link, final double time, Person person, Vehicle vehicle) {
		...
		if (wrapped.getRoute() != null) {
			...
			double bestDepartureTime = preparedTransitSchedule.getNextDepartureTime(wrapped.getRoute(), fromStop, time);
