...
import org.matsim.pt.router.InternalTransitPassengerRoute;
...
public class EvATransitLeastCostPathTree {
	...
	public EvATransitLeastCostPathTree(final Network network, final TransitTravelDisutility costFunction,
...
//		TODO replace it with a flag visited but that would interfere the useage of the MultiNodeDijkstra
	...
	public InternalTransitPassengerRoute getTransitPassengerRoute(final Map<Node, InitialNode> toNodes) {
		...
		while (link != null) {
			...
			if (link.getLine()==null) isTransferLeg = true;
			else {
				transitLineId = link.getLine().getId();
				routeId = link.getRoute().getId();
			}
			...
			if (downstreamLink==null && isTransferLeg) {
				...
				if (routeSegments.size()==0) { // very first leg starts with transfer
					...
					travelTime = routeSegment.getTravelTime();
					toStop = routeSegment.getToStop();
				}
			...
			} else if (downstreamLink.getLine().getId() == link.getLine().getId() && 
					downstreamLink.getRoute().getId() == link.getRoute().getId() ){
				//same route --> update the top routeSegment
				RouteSegment routeSegment = routeSegments.remove(0);
				routeSegments.add(0, new RouteSegment(fromNode.stop.getStopFacility(),
						routeSegment.getToStop(),
						routeSegment.getTravelTime() + travelTime,
						transitLineId,
						routeId));
			}
			...
			if (isTransferLeg) {
				// transfer cost
				if ( ! (this.costFunction instanceof  EvATransitRouterNetworkTravelTimeAndDisutility) ) {
					throw new RuntimeException("TransitTravelDisutility is not instance of "+EvATransitRouterNetworkTravelTimeAndDisutility.class.getSimpleName()
					+". An acc ");
				}

				transferCost += ((EvATransitRouterNetworkTravelTimeAndDisutility) this.costFunction).defaultTransferCost(link,
